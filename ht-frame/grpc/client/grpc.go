...
func InvokeWithResult[C any, T any](
	r *RpcClient[C],
	ctx context.Context,
	handler func(client C, ctx context.Context) (*result.Result[T], error),
...
func InvokeWithTimeoutResult[C any, T any](
	r *RpcClient[C],
	ctx context.Context,
	timeout time.Duration,
	handler func(client C, ctx context.Context) (*result.Result[T], error),
	...
	if err != nil {
		...
		return result.NewResult[T](ctx, 500, err.Error(), def)
	}
	defer conn.Close()
	...
	respCh := make(chan *result.Result[T], 1)
	go func() {
		resp, err := handler(r.newClient(conn), childCtx)
		if err != nil {
			respCh <- result.NewResult[T](ctx, 500, err.Error(), def)
		} else {
			respCh <- resp
		}
	}()
	...
	select {
	case <-ctx.Done():
		return result.NewResult[T](ctx, 500, "服务器响应超时", def)
	case r := <-respCh:
		return r
	}
}
